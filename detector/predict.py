...
def draw(filename,result):
    ...
    output_dir = os.path.join('txts_result', 'resnet50')
    os.makedirs(output_dir, exist_ok=True)  # 如果目录不存在，自动创建
    txt_filename = os.path.join(output_dir, os.path.splitext(os.path.split(filename)[-1])[0] + '_O' + '.txt')
    with open(txt_filename, 'w') as f:  # 打开txt文件，准备写入
        for class_name,lx,ly,rx,ry,ang, prob in res:
            ...
            # 检查是否有负数或坐标超出图像边界，如果有则跳过该行
            if any(val < 0 for val in [x0n, y0n, x1n, y1n, x2n, y2n, x3n, y3n]) or \
                    x0n >= w or x1n >= w or x2n >= w or x3n >= w or \
                    y0n >= h or y1n >= h or y2n >= h or y3n >= h:
                continue

            draw.line([(x0n, y0n),(x1n, y1n)], fill=(255, 0, 0),width=3) # blue  竖线(255, 0, 0)
            draw.line([(x1n, y1n),(x2n, y2n)], fill=(0, 0, 255),width=3) # red    横线
            draw.line([(x2n, y2n),(x3n, y3n)],fill= (255, 0, 0),width=3)
            draw.line([(x0n, y0n), (x3n, y3n)],fill=(0, 0, 255),width=3)

            mid_x1 = (x0n + x2n) / 2.0
            mid_y1 = (y0n + y2n) / 2.0
            mid_x2 = (x1n + x3n) / 2.0
            mid_y2 = (y1n + y3n) / 2.0
            centralX = (mid_x1 + mid_x2) / 2.0
            centralY = (mid_y1 + mid_y2) / 2.0
            f.write(' {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f}\n'.format(x0n, y0n, x1n, y1n, x2n,y2n,x3n,y3n,  centralX, centralY))
            # f.write(f"{x0n}, {y0n}, {x1n}, {y1n}, {x2n}, {y2n}, {x3n}, {y3n}\n")

    # 确保目标文件夹存在
    output_dir = os.path.join('imgs_result', 'resnet50')
    os.makedirs(output_dir, exist_ok=True)  
    img.save(os.path.join(output_dir, os.path.split(filename)[-1]))  
