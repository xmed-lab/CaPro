...
class BasicBlocks(nn.Module):
    def __init__(self, num_blocks, in_channels, out_channels):
        ...
        for i in range(num_blocks):
            lis.append(("{}_block".format(i), UnetResBlock(spatial_dims=2, in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=1, norm_name=norm_params, act_name=act_params)))
...
class UpBlocks(nn.Module):
    def __init__(self, num_blocks, in_channels, out_channels):
        ...
        self.up = UnetUpBlock(spatial_dims=2, in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=1, upsample_kernel_size=2, norm_name=norm_params, act_name=act_params)
        lis = []
        for i in range(num_blocks-1):
            lis.append(("{}_block".format(i), UnetResBlock(spatial_dims=2, in_channels=out_channels, out_channels=out_channels, kernel_size=3, stride=1, norm_name=norm_params, act_name=act_params)))
        self.blocks = nn.Sequential(OrderedDict(lis))
...
class CNA(nn.Module):
    def __init__(self, in_channels, out_channels, k=3):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, k, 1, k//2)
