...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.proj =  nn.Conv2d(channels[-1], 4, kernel_size=1, padding=0)   

        self.out_blocks = nn.Sequential(
            BasicBlocks(1, channels[-1], channels[-1]),
            nn.Conv2d(channels[-1], 1, 3, 1, 1),
            nn.Sigmoid() ####
            )
...
if __name__ == '__main__':
    ...
    H = 64
    bs = 1
    x4 = torch.randn(bs, 512,H//16,H//16).cuda()
    x3 = torch.randn(bs, 256,H//8,H//8).cuda()
    x2 = torch.randn(bs, 128,H//4,H//4).cuda()
    x1 = torch.randn(bs, 64,H//2,H//2).cuda()
    x0 = torch.randn(bs, 32,H,H).cuda() 
